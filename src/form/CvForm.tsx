...
// The friendly form editor covering the full schema. Ported from v1 and used
// inside Step 1 ("Fill it in myself").
...
export function CvForm({
  ...
  const set = (path: (string | number)[], value: unknown) => onChange(setIn(resume, path, value))
  ...
  return (
    <div className="ed-grid">
      <div className="ed-toolbar">
        <button
          ...
          onClick={() => {
            if (
              window.confirm(
                'Clear everything and start with a blank CV? This wipes every field. Save your CV file first if you want to keep a copy.',
              )
            ) {
              onChange(emptyResume())
              onToast('Cleared — every field is now empty.')
            }
          }}
        >
          Clear all
      ...
      <details className="card" open>
        ...
        <div className="photo-row">
          ...
          <div>
            <input
              ...
              onChange={async (e) => {
                ...
                try {
                  ...
                  onToast('Photo stays on this device — it never leaves your browser.')
                } catch {
            ...
            <div className="hint">Optional. Stays on this device (localStorage only).</div>
          </div>
        </div>
        <div className="field-row">
          ...
          <TextField
            ...
            onChange={(v) => {
              ...
              else set(['basics', 'profiles'], [...(b.profiles ?? []), { network: 'LinkedIn', username: v }])
            }}
          />
          <TextField label="Website" value={b.url ?? ''} onChange={(v) => set(['basics', 'url'], v)} />
          <TextField label="Street address" value={loc.address ?? ''} onChange={(v) => set(['basics', 'location', 'address'], v)} />
          <TextField label="Postal code" value={loc.postalCode ?? ''} onChange={(v) => set(['basics', 'location', 'postalCode'], v)} />
          <TextField label="City" value={loc.city ?? ''} onChange={(v) => set(['basics', 'location', 'city'], v)} />
          <TextField label="Country code" value={loc.countryCode ?? ''} onChange={(v) => set(['basics', 'location', 'countryCode'], v)} />
          <TextField label="Birth date" value={b.x_birthDate ?? ''} onChange={(v) => set(['basics', 'x_birthDate'], v)} />
          <TextField label="Residency" value={b.x_residency ?? ''} onChange={(v) => set(['basics', 'x_residency'], v)} />
        </div>
      </details>

      <details className="card" open>
        <summary>Summary paragraphs</summary>
        <StringListEditor items={b.summary ?? []} rows={3} addLabel="Add paragraph" onChange={(items) => set(['basics', 'summary'], items)} />
      </details>

      <details className="card">
        <summary>Highlights</summary>
        <StringListEditor items={b.x_highlights ?? []} addLabel="Add highlight" onChange={(items) => set(['basics', 'x_highlights'], items)} />
      </details>
      ...
      <details className="card">
        ...
        <StringListEditor items={resume.x_coreCompetence ?? []} rows={1} addLabel="Add competence" onChange={(items) => set(['x_coreCompetence'], items)} />
      </details>
      ...
      <details className="card">
        ...
        <StringListEditor items={resume.x_portfolio ?? []} rows={1} addLabel="Add product" onChange={(items) => set(['x_portfolio'], items)} />
      </details>
